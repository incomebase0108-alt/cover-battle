...
## 2026-07-18 — INCOMEBASE04 — 本日終了。次は「戦況HUD(A)」の実装から
- **今日の到達点**: Phase A完了（観戦fps60・S=1/30確定）→ **Phase B第一段(肩越しTPS)がmain入り**(`98b6157`)。
  実機で操作でき、走り/速度差/攻撃の速さ/向き/HP表示まで本人評価「よくなった」
- **次の一手（ここから再開）**: `docs/superpowers/specs/2026-07-18-hud-awareness-design.md` の実装。
  被弾フィードバック（画面の縁＋方向の弧）／ミニマップ（右上・**敵は視界内のみ**）／味方の状態（HP色・倒れている×）。
  **設計は本人承認済み**。`js/net/hud3d.js` として分離し、被弾方向の推定とミニマップ座標変換は純粋関数＋テスト
- **その先**: B（格闘の駆け引き＝防御手段・攻撃の予備動作・間合い・兵種の個性）。**ゲームルールの変更＝1号機のテリトリー**で
  2D版にも影響するため、Aが入って戦況が見えるようになってから設計する（本人と合意済みの順序）
- **1号機への未処理の依頼**: ①`samurai.js` の変更2点のレビュー（animate に running / 近接攻撃の再生速度）
  ②`SNAP_EVERY=32` の採否（17.5→26.6Hz） ③3D操作だけ1.35倍速い件の公平性をどうするか
- 実機検証の注意（再掲）: スマホUIはヘッドレスで再現しきれない。`.touch-only` は `body.touch` 必須／
  viewport幅は再現不可／**実機で2回直らなければキャッシュを疑う**（server.jsに no-store は入れた）

## 2026-07-18 — INCOMEBASE04 — 実機フィードバックでTPSの動きを調整（走り/速度差/攻撃/向き/HP）
- **★星野さん「よくなった」＝方向確定。main へマージ（Phase B試作の第一段が本線入り）**
- 追加調整: **走りを1.35倍(約5.3m/s)** ／ **走りモーションのガタつきを解消**——
  スナップ約18Hz＋100ms補間で生の速度が毎フレーム揺れるため、速度をEMAで均し、
  走りは等速寄り(0.85〜1.25)にクランプ、timeScaleも目標へ寄せる。切替にヒステリシス
- 星野さんの実機指摘を順に対処（ブランチ net/phaseb-tps-proto）:
  ①後退が「奥を向いて手前に歩く」→ **移動中は進行方向へ体を向ける**（攻撃中・停止中は照準向きを保つ）
  ②**CPキャラが後ろ向きで歩く** → 上記を自分だけに入れていたのが原因。**全ユニットに適用**（実測: AI15体中14体が進行方向・後ろ向き0）
  ③走りが無い → **run クリップ対応**（samurai.js の animate に第4引数 running を追加、後方互換）
  ④走り/歩きの速度差 → **サーバは入力ベクトルの長さを速度に掛ける**(entities.js の move、正規化なし)＝改修不要。
    スマホはスティックの倒し具合がそのまま速度。PCは Shift で0.4倍。**アニメ再生速度を実速度に同期**（同期しないと足だけ全力で回り速度差が体感できない）
  ⑤近接の攻撃が遅い → **刀/斬り1.45・槍1.4・大振り1.35・連撃1.3倍**。飛び道具(弓/鉄砲)は構えの間が意味を持つので等速
  ⑥体力表示が無い → 自分のHP/弾/状態を上部中央に（下部は特殊・爆弾ボタンと重なる）
- **実測メモ: 素の移動速度は 3.96m/s**（unit.speed 1.9 × 62.5歩/秒 × S=1/30）＝ジョギング〜走り相当。歩きモーションでは足が滑って当然だった
- **訂正: 「大名/忍者/浪人/衛生兵は run が無い」は誤り。実測したら全8兵種が run を持っていた**
  （samurai.js のコメントのアニメ一覧に載っていなかったための推測。GLBを実際に読んで数えるべきだった）
- スマホ操作の穴も2つ塞いだ: 移動スティックが出ない（`style.display=''` だとページ側の `display:none` が復活）／
  スティックの上でも視点が回る（視点ドラッグの除外に `#touchControls` を追加）

## 2026-07-18 — INCOMEBASE04 — 【真因】**server.js がキャッシュ制御を出していなかった**（実機で修正が届かない）
- **★星野さんの実機で解決を確認（2026-07-18）**。重なりも黒画面も解消。スマホ単体で観戦が回るようになった
- 実機で「右上と重なる」が2回直しても残り、**左上の「？」とも重なる**との報告 → **HUDが上端のまま＝古いHTML**と判断。
  `server.js` の静的配信は **Cache-Control を一切付けていなかった**ので、端末がHTMLを保持し続けていた。
  **HTMLはCSSを内蔵していて `?v` を付けられない**ため、JSだけ ?v を上げても見た目は直らない
- **対処**: `Cache-Control: no-store, must-revalidate` を付与（LAN内の開発サーバなので常に取り直す。
  ゲーム挙動には無関係。※server.jsは1号機テリトリーだが実機検証が成立しない問題なので先に直して報告）
- **重なり自体も根本対処**: 3D観戦では右上の2D用ボタン（🔊・🕹️標準）を使わないので `.hud { display:none }`。
  DOMは残す（client.js が #muteBtn の存在を前提にする）。これで右上は空になり重なりようがない。
  HUDは上端に戻し、左の「？」だけ `padding-left:56px` で避ける
- 検証: `curl -D -` でヘッダを確認、body.touch 強制のスマホ幅で実描画（右上ボタン消滅・重なりなし）

## 2026-07-18 — INCOMEBASE04 — 右上ボタンとの重なりを修正（実機指摘の2回目）＋ヘッドレス検証の限界
- **原因**: 右上の「🕹️標準」は `.touch-only`＝**body.touch のときだけ出る**。ヘッドレスでは付かないので
  右上が空いて見え、**重なりを再現できていなかった**（前回「直した」と言ったのに実機で残っていた理由）
- **対処**: 横方向で避けるのをやめ、**HUDを右上ボタンの一段下へ**（`top: calc(env(safe-area-inset-top,0px) + 58px)`）。
  端末ごとにボタン幅が変わるので、行を分けるのが確実。狭い画面では砦%表示も短縮
- **ヘッドレス検証の限界（重要）**: `<meta viewport width=device-width>` のため**ウィンドウサイズを変えても
  ページのビューポートは492pxのまま**（--window-size=390でも --force-device-scale-factor=1 でも変わらず）。
  つまり**実機の390px幅は再現できない**。CSSメディアクエリ(600px以下)は効くので配色や省略は確認できるが、
  **折り返しの最終確認は実機が必要**。検証時は `document.body.classList.add('touch')` を強制すること
- 真っ暗の件は解決（本人確認済み）。原因は①停止中に誰も描いていなかった ②`?v=1` のままでスマホが古いJSを掴んでいた

## 2026-07-18 — INCOMEBASE04 — スマホ幅のHUD崩れを修正（本人指摘: 右上に文字が重なる）
- 原因: `#hud3d` に幅の制限が無く、スマホ幅で文字列が画面外へ流れて右上のミュート等に重なっていた
  （占有領域を実測: 左上 `.help-btn` 44px @10,10 ／ 右上 `.hud-row` の2ボタン）
- 対処: `right` と左右padding(62/104px)で両端を避ける・600px以下は文字と余白を詰めタイトルを落とす・
  停止中の警告文もスマホでは短縮・**文字に下地**（空や砂地の明るい背景では影だけだと赤字/黄字が沈む）
- **副産物: 試合が止まっている間は画面が真っ黒だった**。frame()が来ない＝誰も描かないため。
  **1回 render しても表示は残らない**（WebGLの描画結果はフレームをまたがない）ので、停止中だけ **rAFで描き続ける**
- 検証: スマホ幅390×844で「停止中」「試合中」の両方を実描画。両方2行に収まり重なりなし。tests 166/166 PASS
- 検証ハーネスの罠（またやった）: 注入スクリプトの `
` が生の改行になり **SyntaxError で丸ごと死んで**
  「ロビーが閉じない」と誤診しかけた。**実装を疑う前に生成物の中身を見る**

## 2026-07-18 — INCOMEBASE04 — **スナップ19Hzの原因を実測で特定**（1号機の観測への回答）
- 1号機が観測した「スナップ約19Hz（設計30Hz）」の原因は**計測プロセスの同居ではなく `server.js` の作り**:
  `sinceSnap += STEP(16)` は16刻みでしか増えず、`>= SNAP_EVERY(33)` を跨ぐのは**48ms(3ステップ)**
  ＝**理論上限20.8Hz**で30Hzは構造的に出ない
- **実測（wsで直接繋いでsnap到着間隔を8秒測定）**: `SNAP_EVERY=33` → **17.5Hz**(中央値49ms・最頻48ms×41)／
  `=32` に変えると → **26.6Hz**(中央値32.4ms・最頻32ms×100)。**1文字で+52%**。残差(48msが34回)は setInterval のタイマー遅延
- server.js は1号機のテリトリーなので**変更は戻して実測データと修正案だけ渡した**（main無変更）。採否と帯域1.5倍の判断は1号機
- 計測の型: `ws` で直接接続 → pick → start → snap の `process.hrtime.bigint()` 差分をヒストグラム化。
  ブラウザを介さないので描画負荷に汚されない（1号機の `tools/probe.js` と同趣旨）
...
## 2026-07-18 — INCOMEBASE04 — **Phase B試作: 肩越し(TPS)操作ビュー**（ブランチ net/phaseb-tps-proto）
- 星野さんの「操作視点をプレイヤー3D視点にできないの？」に対する試作。**方式=TPS(肩越し)を本人が選択**
- **サーバ改修ゼロ**で成立: 送っているのは移動ベクトルと照準角のワールド絶対値だけなので、
  カメラ基準へ回す変換を1枚かませるだけでよい。本体改修は **client.js のフック1点**
  (`window.NetInput.map`。未定義なら2D版は挙動不変＝Phase Aのフックと同じ流儀)
- 新規 `js/net/input3d.js`(純粋関数) + `tests/input3d.test.js`(7件)。**TDDで先にテストを書いた**。
  右ベクトルは目分量でなく **前方×up** で導出（[[feedback-screen-right-is-look-cross-up]]の3回再発を踏まえ符号テスト必須）
- render3d.js に TPSカメラ(背後4.6m/目線1.5m/右0.75mずらし)・操作トグル、netclient3d.html にクロスヘア
- **実測(実試合に参加してキー入力→移動距離を計測)**: W→前2.22m(右-0.01) / D→右2.69m(前-0.01)＝**反転なし**。
  送信値もカメラの前方/右ベクトルと一致。tests 174/174 PASS。**2D版の回帰も実描画で確認**(NetInput未定義のまま従来動作)
- 検証ハーネスの罠2つ: ①**試合開始前は Net.map が無く描画フックが呼ばれない**→NET を取れないので参加判定は時間ベースに
  ②キーを離した直後は送信済み入力がまだ反映中→**次の計測の前に900ms静止**しないと前の移動が食い込む(NG誤判定の原因だった)
- 星野さんへ試作報告(スクショ＋実測表)を #3d戦国 へ。**次に決めたいこと=カメラ距離/2D版との公平性/照準補正**
