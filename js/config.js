...
const CONFIG = {
  ...
  // サドンデス（決着の保証）：発動すると両軍の城が自動で崩れ始める。
  // 発動条件＝①開始から afterMs 経過（強制） ②どちらかの総大将が討死。
  // 以前は3分・毎秒45（城800が約18秒で全壊）かつ無表示で、「攻撃されてないのに
  // 城の耐久が減るバグ」に見えていた。画面表示とセットで緩やかに。
  suddenDeath: {
    afterMs: 300000,   // 発動までの時間（5分。HP増量で試合が伸びたぶん3分→5分）
    drainPerSec: 20,   // 発動中に両軍の城が毎秒失う耐久（旧45は急すぎた）
    warnMs: 30000,     // 発動この時間前からカウントダウンを表示
  },
