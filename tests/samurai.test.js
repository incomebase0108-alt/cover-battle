...
s.test("鉄砲は5連発：撃ち切ると装填に入る", (t) => {
  const { sb, game } = newGame(0);
  const u = new sb.Unit(1500, 300, "blue");
  u.applyClass("gunner"); // 鉄砲
  u.stamina = 1e9; // 弾倉機構の検証では体力ゲートを無効化
  game.bullets = [];
  t.equal(u.ammo, 5, "装填数は5");
  for (let i = 0; i < 5; i++) { u.cooldown = 0; u.tryShoot(game); }
  t.greaterThan(game.bullets.length, 4, "5発撃てる");
  t.equal(u.reloading, true, "撃ち切ったら装填に入る");
});
...
s.test("大将が救出されず一定時間経つと討死(状態2)。城は自動では減らない（サドンデス廃止）", (t) => {
  const { sb, game } = newGame(0);
  const g = game.generalOf("blue");
  g.alive = false; g.downed = true;
  game._genDownMs = { blue: (sb.CONFIG.morale.discardMs + 100), red: 0 };
  t.equal(game.generalStatus("blue"), 2, "討死(状態2)になる");
  const blueFort = game.map.baseOf("blue").hp;
  const redFort = game.map.baseOf("red").hp;
  game._update(16);
  t.equal(game.map.baseOf("blue").hp, blueFort, "討死しても城の耐久は自動で減らない");
  t.equal(game.map.baseOf("red").hp, redFort, "敵の城も同様（攻撃以外で城は減らない）");
});

s.test("総大将の討死で決着：敵将なら勝ち(general)・味方将なら負け", (t) => {
  {
    const { sb, game, ended } = newGame(0);
    const g = game.generalOf("red");
    g.alive = false; g.downed = true;
    game._genDownMs = { blue: 0, red: sb.CONFIG.morale.discardMs + 100 };
    game._update(16);
    t.ok(ended.value && ended.value.win === true, "敵総大将の討死＝勝利");
    t.equal(ended.value.reason, "general", "決着理由は『総大将討ち取り』");
  }
  {
    const { sb, game, ended } = newGame(0);
    const g = game.generalOf("blue");
    ...
    game._genDownMs = { blue: sb.CONFIG.morale.discardMs + 100, red: 0 };
    game._update(16);
    t.ok(ended.value && ended.value.win === false, "味方総大将の討死＝敗北");
    t.equal(ended.value.reason, "general", "決着理由は『総大将討ち取り』");
  }
});
...
s.test("息切れでも攻撃は出る（威力だけ下がる＝無音の不発を廃止）", (t) => {
  const { sb, game } = newGame(0);
  const a = new sb.Unit(1500, 300, "blue");
  a.applyClass("ashigaru");
  ...
  a.cooldown = 0;
  a.tryShoot(game);
  const tiredDmg = enemy.maxHp - enemy.hp;
  t.greaterThan(tiredDmg, 0, "息切れでも攻撃が当たる（不発にならない）");
  // 回復後の一撃と比べて威力が落ちている。
  const enemy2 = new sb.Unit(1530, 300, "red");
  game.units = [a, enemy2];
  a.stamina = a.maxStamina;
  a.cooldown = 0;
  a.meleeRecheck = null;
  a.tryShoot(game);
  const fullDmg = enemy2.maxHp - enemy2.hp;
  t.lessThan(tiredDmg, fullDmg, "息切れ中は威力が下がる");
  const minMul = sb.CONFIG.stamina.tiredDamageMul;
  t.ok(tiredDmg >= fullDmg * minMul - 0.01, "威力の下限は tiredDamageMul で保証");
});
...
s.test("大筒は着弾点で爆弾級の爆発（周囲の敵にもダメージ・味方は巻き込まない）", (t) => {
  const { sb, game } = newGame(0);
...
s.test("鉄砲は5連発＋特殊『早合』で装填を即完了", (t) => {
  ...
  const u = new sb.Unit(1500, 300, "blue");
  u.applyClass("gunner");
  t.equal(u.magSizeVal(), 5, "鉄砲の弾倉は5発");
  u.startReload(game);
  t.equal(u.reloading, true, "装填が始まる");
  u.useAbility(game); // 早合
  t.equal(u.reloading, false, "早合で装填が即完了する");
  t.equal(u.ammo, u.magSizeVal(), "弾倉が満タンになる");
  t.greaterThan(u.abilityCd, 0, "早合はクールダウンに入る");
});

s.test("高速弾が敵をすり抜けない（1フレームの移動を小刻みに判定）", (t) => {
  const { sb, game } = newGame(0);
  const enemy = new sb.Unit(1600, 300, "red");
  game.units = [enemy];
  // 1フレームで150px進む高速弾（敵の判定幅約20pxを大きく飛び越える速度）。
  const b = new sb.Bullet(1500, 300, 1, 0, "blue", { damage: 16, speed: 150, life: 1000 });
  game.bullets = [b];
  b.update(16, game);
  t.lessThan(enemy.hp, enemy.maxHp, "通り道の敵に当たる（すり抜けない）");
  t.equal(b.dead, true, "命中した弾は消える");
});

s.test("城は中心から離れた位置（石垣の見た目相当）でもダメージが入る", (t) => {
  const { sb, game } = newGame(0);
  const red = game.map.baseOf("red");
  const before = red.hp;
  const offset = sb.CONFIG.base.coreHitRadius - 5; // 旧判定(coreR=30)では外れる距離
  const b = new sb.Bullet(red.x - offset, red.y, 0, 0, "blue", { damage: 16, speed: 0, life: 1000 });
  game.bullets = [b];
  b.update(16, game);
  t.lessThan(red.hp, before, "石垣相当の範囲でも城の耐久が減る");
});

s.test("近接は振りの途中で間合いに入った敵にも当たる", (t) => {
  const { sb, game } = newGame(0);
  const a = new sb.Unit(1500, 300, "blue");
  a.applyClass("ashigaru"); // 刀
  const enemy = new sb.Unit(1800, 300, "red"); // 振り始めは間合いの外
  game.units = [a, enemy];
  a.aim = 0;
  a.cooldown = 0;
  a.tryShoot(game);
  t.equal(enemy.hp, enemy.maxHp, "間合いの外では当たらない");
  enemy.x = 1530; // 振っている間に間合いへ入ってきた
  a.update(400, game); // 振りの途中判定（swingMsの45%地点）を経過させる
